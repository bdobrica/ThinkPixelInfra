...
func verifyDomain(domain string, path string, salt string, token string) {
    ...
    logger.Infof("Verifying domain %s%s with token %s (attempts: %d)", domain, path, token, maxAttempts)

    // Atomic counter to track attempts
    var attemptCounter int32

    // Immediate first attempt
    executeVerification(client, url, domain, path, salt, token, &attemptCounter, maxAttempts)
...
func executeVerification(client http.Client, url string, domain string, path string, salt string, token string, attemptCounter *int32, maxAttempts int32) {
    req, err := http.NewRequest("GET", url, nil)
    if err != nil {
        logger.Errorf("Failed to create verification request: %v", err)
        return
    }

    resp, err := client.Do(req)
    if err != nil {
        logger.Errorf("Verification request failed for %s: %v", url, err)
    ...
    if resp.StatusCode != http.StatusOK {
        logger.Infof("Verification failed for %s with status code %d", url, resp.StatusCode)
        checkFinalAttempt(domain, path, token, attemptCounter, maxAttempts)
        return
    }
    ...
    if err := json.NewDecoder(resp.Body).Decode(&verificationResponse); err != nil {
        logger.Errorf("Failed to decode verification response: %v", err)
        checkFinalAttempt(domain, path, token, attemptCounter, maxAttempts)
        return
    }

    verificationToken, _, err := db.GetPendingTokenDetails(verificationResponse.Domain, verificationResponse.Path)
    if err != nil {
        logger.Errorf("Failed to retrieve verification token details for domain: %s, path: %s, salt: %s: %v", verificationResponse.Domain, verificationResponse.Path, salt, err)
        checkFinalAttempt(domain, path, token, attemptCounter, maxAttempts)
        return
    }

    saltedToken := saltMessage(verificationToken, salt)
    if saltedToken == verificationResponse.SaltedToken {
        if err := db.VerifyToken(token, domain, path); err != nil {
            logger.Errorf("Failed to mark token as verified: %v", err)
        } else {
            logger.Infof("Verification succeeded for %s", url)
        }
    } else {
        checkFinalAttempt(domain, path, token, attemptCounter, maxAttempts)
    }
...
func checkFinalAttempt(domain, path, token string, attemptCounter *int32, maxAttempts int32) {
    if atomic.AddInt32(attemptCounter, 1) == maxAttempts {
        if err := db.FailToken(token, domain, path); err != nil {
            ...
            logger.Infof("Final verification attempt failed. Token marked as failed for %s%s", domain, path)
        }
    }
}
