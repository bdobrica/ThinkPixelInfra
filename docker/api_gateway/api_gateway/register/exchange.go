...
func exchangeTokenForAPIKey(domain, path, saltedToken string) (string, string, error) {
    ...
    verificationToken, requestSalt, err := db.GetVerifiedTokenDetails(domain, path)
    if err != nil {
        return "", "", fmt.Errorf("Failed to get verification token details: %v", err)
    }

    if saltedToken != saltMessage(verificationToken, requestSalt) {
    ...
    if err := db.ActivateAPIKey(verificationToken, apiKey); err != nil {
