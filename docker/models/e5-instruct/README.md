...
- `MODEL_PATH`: Path to the model weights. Default: `/app/multilingual-e5-large-instruct`
- `MODEL_ZMQ_CLIENT_ADDR`: ZMQ socket address where FastAPI threads are connecting to. Default: `ipc:///tmp/multilingual-e5-large-instruct.client`
- `MODEL_ZMQ_WORKER_ADDR`: ZMQ socket address where the Model Workers are connecting to. Default: `ipc:///tmp/multilingual-e5-large-instruct.worker`
...
2a828b375f8f   great_mahavira   0.14%     1.602GiB / 15.54GiB   10.31%    946B / 126B   0B / 152kB   16
```
...
```
CONTAINER ID   NAME               CPU %     MEM USAGE / LIMIT     MEM %     NET I/O       BLOCK I/O    PIDS
74b351bee18e   laughing_jemison   0.18%     6.077GiB / 15.54GiB   39.10%    876B / 126B   0B / 152kB   34
```
...
```bash
curl \
    -X POST "http://e5-instruct:8000/infer" \
