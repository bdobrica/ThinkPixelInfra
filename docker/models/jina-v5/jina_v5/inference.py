"""
Inference module for jina-embeddings-v5-text-small-retrieval: Retrieval-Targeted Embedding Distillation model
with ZeroMQ communication.

This module provides the core inference functionality for the jina-embeddings-v5-text-small-retrieval model
using ONNX Runtime. It handles model loading, batch processing, and ZeroMQ-based
...
from .postprocess import build_results, load_special_tokens
...
def load_model():
    ...
    # Load special tokens for postprocessing
    load_special_tokens(lambda: tokenizer.special_tokens_map)
        ...
        try:
            ...
            for index in range(0, len(text_items), MODEL_BATCH_SIZE):
                ...
                batch_input = tokenizer(
                    ...
                    truncation=True,
                    return_tensors="np",
                    max_length=min(MODEL_MAX_TOKENS, 32768),  # Cap to model's max input length
                )
