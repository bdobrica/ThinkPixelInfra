...
- `MODEL_PATH`: Path to the model weights. Default: `/app/jina-embeddings-v5-text-small`
- `MODEL_MAX_TOKENS`: Maximum number of tokens that the tokenizer will produce (<=`32768`>). Default: `8192`.
- `MODEL_ZMQ_CLIENT_ADDR`: ZMQ socket address where FastAPI threads are connecting to. Default: `ipc:///tmp/jina-embeddings-v5-text-small.client`
- `MODEL_ZMQ_WORKER_ADDR`: ZMQ socket address where the Model Workers are connecting to. Default: `ipc:///tmp/jina-embeddings-v5-text-small.worker`
...
aa70ac7c2b71   angry_nobel   0.01%     2.511GiB / 15.54GiB   16.15%    4.32kB / 3.24kB   8.19kB / 160kB   26
```
...
```
CONTAINER ID   NAME            CPU %     MEM USAGE / LIMIT     MEM %     NET I/O           BLOCK I/O         PIDS
5a786b6deee6   youthful_pare   0.22%     9.611GiB / 15.54GiB   61.84%    5.36kB / 4.09kB   19.4MB / 86.8MB   68
```
...
```bash
curl \
    -X POST "http://jina-v5:8000/infer" \
